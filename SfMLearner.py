        ...
        with tf.name_scope("compute_loss"):
            ...
            for s in range(opt.num_scales):
                ...
                for i in range(opt.num_source):
                    ...
                    #curr_proj_error = tf.add(curr_proj_error,tf.multiply(0.5,tf.subtract(1,tf.image.ssim(curr_tgt_image,curr_proj_image,max_val=255))))
                    #sys.exit(0)
                    ...
                    if opt.explain_reg_weight > 0:
                        ...
                        pixel_loss += tf.reduce_mean(curr_proj_error)
                        #tmp_curr_proj_image = tf.image.convert_image_dtype(curr_proj_image,tf.float32)
                        #tmp_curr_tgt_image = tf.image.convert_image_dtype(curr_tgt_image,tf.float32)
                        #pixel_loss += tf.multiply(0.5,tf.subtract(1.0,tf.image.ssim(tmp_curr_tgt_image,tmp_curr_proj_image,max_val=1)))
